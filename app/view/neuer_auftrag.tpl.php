...
<div id="content_box_top">
    ...
    <p class="copy_12">
        <strong>
            Eine Fertigstellung Ihrer Grundrisse innerhalb 48 Stunden kann nur gewährleistet werden, wenn
            der Auftragseingang bis spätestens 16:00 Uhr erfolgt und keine weiteren Absprachen notwendig sind.
        </strong>
    </p>
    <p class="copy_12" style="color: red;">
        <strong>
            Die Rechnungsstellung erfolgt per E-Mail an Ihre hinterlegte E-Mail-Adresse <?php echo $user->getLogin() ?>.
        </strong>
    </p>
</div>
        ...
        <form action="auftrag_anlegen" method="post" enctype="multipart/form-data" >
            ...
            <div class="form_float_box">
				...
				<?php  if ($user->getId() == 53) { ?>
                    <li><input <?php if($_SESSION['job_request']['design'] == 'Volksbank' || !$_SESSION['job_request']['design']) { echo 'checked="checked"'; }?> type="radio" name="design" id="cb_design_1" value="Volksbank" /><label for="cb_design_1" class="cb_label"><strong>Volksbank</strong></label></li>
                <?php } ?>  
...
<h2 class="form_headline">Optionen (ohne Aufpreis)</h2>
            ...
            <p class="copy_12">
                ...
                Wählen Sie Ihre zu bearbeitenden Dateien mit "Durchsuchen" aus, und <strong>klicken Sie anschließend auf "hochladen"</strong>, damit die Datei in untenstehender Liste erscheint.
           ...
           <p class="copy">Datei wird hochgeladen. Dies kann je nach Größe der Datei und Geschwindigkeit Ihrer Internetverbindung einige Momente dauern.</p>
            ...
            <textarea name="bemerkung" style="font-size:12px;" cols="37" rows="5" ><?php if(isset($_SESSION['job_request']['bemerkung'] )) { echo $_SESSION['job_request']['bemerkung']; } elseif($user->getId() == 53) { echo 'unter den Grundriss den Vermerk: (unverbindliche Illustration); Alle Raumbezeichnungen ausschreiben;'; }?></textarea>
            <hr />
            <input type="submit" value="" name="form_submit" id="send_job"/>
        </form>
