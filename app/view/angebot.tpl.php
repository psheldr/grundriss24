...
<div id="content_box_top">
    <p class="fett_orange_italic" >Angebotsanfrage - <?php echo $_SESSION['job_request']['auftragsnummer']?></p>
    <p class="copy_12">
...
<div id="content_box_bottom">
    ...
    <div id="form_box_auftrag">
        <form action="angebot_anlegen" method="post" enctype="multipart/form-data" >
...
<hr />
            ...
            <label for="job_objektinfo" class="cb_label" class="cb_label_breit">mit Objektinfo: Text, der als Über- oder Unterschrift mit auf die Zeichnung soll.
                ...
                ggfs. in "Bemerkungen" oder mit auf die Vorlage.</label><br style="margin-bottom: 10px;"/>
            ...
            <label for="job_raumbezeichnungen" class="cb_label">mit Raumbezeichnungen: auf dem Grundriss vermerkte Bezeichnungen werden auch
                im gestalteten Grundriss eingetragen. (z.B.: Schlafen, Wohnen, Küche)</label><br style="margin-bottom: 10px;"/>
            ...
            <h2 style="display:none" class="form_headline">Wie findet die Übermittlung der Vorlagen statt?</h2>
            <span style="display:none" class="reg_label">Ich sende die Grundrisse per:</span>
            <input style="display:none" type="radio" class="" value="Datei-Upload" name="versand" id="cb_transfer_1" checked="checked" <?php if($_SESSION['job_request']['versand'] == 'Datei-Upload') { echo 'checked="checked"'; }?> /><label style="display:none" for="cb_transfer_1" class="cb_label">Datei-Upload</label>
            <input style="display:none" type="radio" class="" value="Fax" name="versand" id="cb_transfer_2" <?php if($_SESSION['job_request']['versand'] == 'Fax') { echo 'checked="checked"'; }?> /><label style="display:none" for="cb_transfer_2" class="cb_label">Fax</label>
            <input style="display:none" type="radio" class="" value="Post" name="versand" id="cb_transfer_3" <?php if($_SESSION['job_request']['versand'] == 'Post') { echo 'checked="checked"'; }?> /><label style="display:none" for="cb_transfer_3" class="cb_label">Post</label>
            <p style="display:none" class="copy_12">Falls Sie uns Ihre Unterlagen per Fax oder Post zusenden, erscheint nach "Auftrag absenden" ein Deckblatt zum Ausdrucken.
            ...
            <p class="copy_12">
                Laden Sie hier die Grundrisse hoch, für welche wir Ihnen ein unverbindliches Angebot erstellen sollen.
            ...
            <!--<p><?//=displayExts($ext_array);?></p>-->
            <input type="file" id="file_upload" name="file_upload"  />
            <?php
            if ($dateien != NULL) {
            foreach($dateien as $datei) { ?>

            <p class="uploaded_data"><?php echo $datei->getDateiname() ?> <a class="btn_delete" href="index.php?action=remove_file&pre=angebot&file_id=<?php echo $datei->getId(); ?>"></a></p>
            <?php }}
            ?>
            <!--<a name="test" href="javascript:$('#file_upload').uploadifyUpload();" id="btn_upload_now"></a><br />-->
            <input type="submit" value="" id="btn_upload_now" name="upload_btn" onclick="javascript: showLoader()" />
           ...
           <p class="copy">Datei wird hochgeladen. Dies kann je nach Größe der Datei und Geschwindigkeit Ihrer Internetverbindung einige Momente dauern.</p>
            ...
            <textarea name="bemerkung" cols="37" rows="5" ><?php if($_SESSION['job_request']['bemerkung'] != '') { echo $_SESSION['job_request']['bemerkung']; }?></textarea>
            <hr />
            <input type="submit" value="" id="send_angebot"/>
        </form>
